...
class ColumnValuesToNotContainSpecialCharacters(ColumnMapMetricProvider):
    ...
    # ADD forbid_double_space here
    condition_value_keys = ("allowed_characters", "forbid_double_space")

    # This method defines the business logic for evaluating the metric when using a PandasExecutionEngine
    @column_condition_partial(engine=PandasExecutionEngine)
    def _pandas(cls, column, allowed_characters: list | set, **kwargs):
    ...
    def _spark(cls, column, allowed_characters: list | set, **kwargs):
        def not_contain_special_character(val, *special_characters):
            special_characters = [
    ...
    def _sqlalchemy(
        ...
        forbid_double_space: bool = False,
        **kwargs,
    ):
        import string
        ...
        # ❌ reject double spaces if flag is on
        if forbid_double_space:
            predicates.append(~column.like("%  %"))

        escape_char = "\\"
        for ch in disallowed:
            like_char = ch
            if ch in ["%", "_", escape_char]:
                like_char = escape_char + ch

            pattern = f"%{like_char}%"
            predicates.append(~column.like(pattern, escape=escape_char))
...
class ExpectColumnValuesToNotContainSpecialCharacters(ColumnMapExpectation):
    ...
    success_keys = ("mostly", "allowed_characters", "forbid_double_space")

    default_kwarg_values = {"mostly": 1, "allowed_characters": [], "forbid_double_space": False}
